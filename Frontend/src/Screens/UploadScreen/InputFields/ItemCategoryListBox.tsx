...
import { itemCategories, itemCategoryMap } from "../../../Data/utils";
    ...
    return (
        <Listbox value={value} onChange={(val) => { setHandler(val); handleChange() }}>
            <div className="relative mt-1">
                <Listbox.Button className={` ${inBrowse ? `ml-12 w-[260px]` : ``} relative w-full py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-orange-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500 sm:text-sm`}>
                    <span className="block truncate">{itemCategoryMap[value.name]}</span>
