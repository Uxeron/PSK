...
import { itemConditionMapBrowse, itemCategoriesBrowse, itemCategoryMapBrowse, itemConditionBrowse } from "../../Data/utils";
...
export const ItemConditionListBoxBrowse = (props : Props) => {
    const {setHandler, value, inBrowse} = props;

    return(
        <Listbox value={value} onChange={(val) => { setHandler(val); }}>
                    <div className="relative mt-1">
                    <Listbox.Button className={ ` ${inBrowse ? `ml-12 w-[260px]` : ``} ml-12 w-[260px] relative w-full py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-orange-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500 sm:text-sm`}>
                            <span className="block truncate">{itemConditionMapBrowse[value.name]}</span>
                        ...
                        >
                            <Listbox.Options className="absolute w-full z-50 py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                                {itemConditionBrowse.map((itemCondition, idx) => (
                                    <Listbox.Option
                                        key={idx}
                                        className={({ active }) =>
                                    ...
                                    >
                                        {({ selected }) => (
                                            <>
                                                ...
                                                >
                                                    {itemConditionMapBrowse[itemCondition.name]}
                                                </span>
                                                {selected ? (
