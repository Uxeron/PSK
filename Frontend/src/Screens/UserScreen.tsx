...
import { ExpandedUser } from "../Data/model";
...
export const UserScreen = () => {
    ...
    useEffect(() => {
        const initalize = async () => {
            try {
                await getAccessTokenSilently().then((token) => { UserService.getById({ accessToken: token, id: userId ?? '' }).then((val) => setData(val.data)) })
            } catch (e) {
                console.log(e);
            }
        };
    ...
    return (
        <>
            ...
            <div className="m-auto min-w-[1000px] max-w-7xl grid grid-cols-4 gap-12 mb-16">
                {data.listedItems ? data.listedItems.map((item) => { return <ItemCard key={item.itemId} id={item.itemId} name={item.name} description={item.description} image={item.image} isGivenAway={item.isGivenAway} /> }) : undefined}
            </div>
